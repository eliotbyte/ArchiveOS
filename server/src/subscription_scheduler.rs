...
pub fn spawn(config_dir: PathBuf, vault_name: String) {
    tokio::spawn(async move {
        if let Err(err) = run_once(&config_dir, &vault_name).await {
            tracing::warn!("subscription scheduler initial run failed: {err}");
        }
        ...
        loop {
            interval.tick().await;
            if let Err(err) = run_once(&config_dir, &vault_name).await {
                tracing::warn!("subscription scheduler poll failed: {err}");
            }
        }
    });
}
...
async fn run_once(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let jobs = vault.process_due_subscriptions()?;
        if !jobs.is_empty() {
            tracing::info!(count = jobs.len(), "subscription scheduler created jobs");
        }
        Ok::<(), archiveos_contract::VaultError>(())
    })
    .await
    .expect("subscription scheduler task panicked")?;
    Ok(())
}

fn poll_secs() -> u64 {
    std::env::var("ARCHIVEOS_SUBSCRIPTION_POLL_SECS")
        .ok()
...
pub fn enabled() -> bool {
    match std::env::var("ARCHIVEOS_SUBSCRIPTION_SCHEDULER").as_deref() {
